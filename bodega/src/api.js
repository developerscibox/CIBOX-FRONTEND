...
export const api = {
  ...
  // 409 si ya fue tomado o si está asignado a otra persona.
  aceptar: (id) => req(`/orders/admin/${id}/aceptar`, { method: "POST" }),
  // GET /orders/admin/pickers (users.manage) → personal de preparación activo (operator|admin)
  ...
  // Preparación (gate orders.prepare): avance persistido, faltante/dañado y cierre de empaque (→ready).
  ...
  // GET /admin/gerencia?from&to → centro de mando (ventas, operación, equipo, productos, cobros, inventario)
  gerencia: (params = {}) => {
    const clean = Object.fromEntries(Object.entries(params).filter(([, v]) => v != null && v !== ""));
    const qs = new URLSearchParams(clean).toString();
  ...
  // GET /reports/valorizacion → { por_categoria:[], totales:{valor_costo,valor_venta,skus,sin_costo} }
  ...
  // GET /config/modules (público) → { enabled:["web","bodega","gerencia"] }
  ...
  // POST /inventory/counts  { scope:{type:"category"|"all", value, label} }
