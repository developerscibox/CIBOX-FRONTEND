...
const TYPE_COLOR = { boleta: "#E6007E", factura: "#7A1B73", nota_credito: "#FF3DA6" };
...
export default function Documentos() {
  ...
  function exportPDF() {
    ...
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Documentos SII · Bodega 12</title>
    <style>
      *{box-sizing:border-box} body{font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#2a1022;margin:0;padding:28px}
      .hd{display:flex;align-items:center;gap:14px;border-bottom:3px solid #E6007E;padding-bottom:14px;margin-bottom:18px}
      ...
      th{background:#fbeef6;color:#7A1B73;font-size:11px;text-transform:uppercase;letter-spacing:.5px}
      td.r,th.r{text-align:right} td.b{font-weight:700}
      .ft{margin-top:20px;color:#8a6e80;font-size:11px;text-align:center}
      @media print{body{padding:0}}
    </style></head><body>
      <div class="hd"><img src="${origin}/logo-bodega12.png" alt="Bodega 12" onerror="this.style.display='none'"/>
        <div><h1>Documentos tributarios (SII)</h1><div class="sub">Bodega 12 · Lo Espejo — emitido el ${fmt(new Date().toISOString())}${status ? " · filtro: " + (DOC_STATUS[status]?.label || status) : ""}</div></div></div>
      <div class="kpis">
      ...
      <div class="ft">Documento generado desde el panel de Bodega 12. Facturación en modo stub (folios STUB-…).</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 16 }}>
        <Kpi ic="🧾" bg="#fce7f3" value={res.loading ? "…" : agg.totals.count.toLocaleString("es-CL")} label="Documentos emitidos" />
      ...
      <div className="card" style={{ marginBottom: 0 }}>
        <div className="card-h">
          ...
          <span className="badge" style={{ background: "#fce7f3", color: "#9d174d" }}>{res.loading ? "…" : `${items.length}`}</span>
        </div>
        <table>
          <thead>
